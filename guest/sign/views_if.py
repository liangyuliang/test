...
def add_guest(request):
    ...
    etime = str(event_time).replace('T',' ').replace('Z','').split("+")[0]
    timeArray = time.strptime(str(etime),"%Y-%m-%d %H:%M:%S")
...
def user_sign(request):
    ...
    if n_time >=e_time:
        return JsonResponse({'status':10024,'mesage':'event has started'})
    result = Guest.objects.filter(phone=phone)
    if not result:
        return JsonResponse({'status':10025,'message':'user phone null'})
    result = Guest.objects.filter(event_id=eid,phone=phone)
    if not result:
        return JsonResponse({'status':10026,'message':'user id not participate in the conference'})
